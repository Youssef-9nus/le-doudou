...
export default function AdminPage() {
  ...
  // Upload de plusieurs images s?lectionn?es d'un coup (galerie multi-s?lection)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // reset pour pouvoir re-s?lectionner les m?mes fichiers si besoin
    e.target.value = "";
  };
  ...
  const ouvrirNouveauProduit = () => {
    setFormData({
      ...
      couleurs: [],
      tailles: ["XS", "S", "M", "L", "XL"],
      description: "Un essentiel intemporel pour compl?ter votre garde-robe.",
      nouveaute: false,
      soldout: false,
      images: [],
      stock: 0,
                ...
                <div key={p.id} className="border border-white/10 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <div className="flex gap-1.5 flex-wrap pt-1">
                      ...
                      {(p.couleurs || []).length === 0 && (
                        <span className="text-white/25 text-xs">Aucune couleur renseign?e</span>
                      )}
                    </div>
