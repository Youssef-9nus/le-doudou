...
export default function ProduitPage({
  ...
  useEffect(() => {
    const fetchProduit = async () => {
      ...
      const couleurs = normaliserListe(data.couleurs);
      const tailles = normaliserListe(data.tailles);

      setProduit(data);
      setCouleurChoisie(couleurs[0] ?? "Couleur unique");
      setTailleChoisie(tailles[0] ?? "Taille unique");
  ...
  const couleursDisponibles = produit ? normaliserListe(produit.couleurs) : [];
  const taillesDisponibles = produit ? normaliserListe(produit.tailles) : [];
  const couleursAffichees = couleursDisponibles.length > 0 ? couleursDisponibles : ["Couleur unique"];
  const taillesAffichees = taillesDisponibles.length > 0 ? taillesDisponibles : ["Taille unique"];
  ...
  const handleAjouterAuPanier = () => {
    ...
    if (!produit) return;
    ajouterAuPanier(produit, couleurChoisie || couleursAffichees[0], tailleChoisie || taillesAffichees[0]);
    setAjoute(true);
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-3">
            ...
            {/* Vignettes affichees si plusieurs images */}
            {images.length > 1 && (
